...
export default (scene, data) => {
  ...
  loader.load(data.deltaCamp3DSymbol.publicURL, function (symbolObject3d) {
    // var meshes = symbolObject3d.children
    //
  ...
  function update(time, yScrollPos) {
    ...
    if (createdSubject) {
      ...
      var newY = THREE.Math.lerp(originalSymbolPos.y, originalSymbolPos.y, yScrollPos * 0.001)
      var newZ = THREE.Math.lerp(originalSymbolPos.y, originalSymbolPos.y, yScrollPos * 0.001)

      createdSubject.position.copy(new THREE.Vector3(newPos.x, newY * 2, newZ * 2))

      createdSubject.rotation.y += 0.001;
    }
